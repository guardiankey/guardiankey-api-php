<?php
/* ======= THIS FILE WAS MODIFIED FOR INTEGRATION WITH GUARDIANKEY =========
** Zabbix
...
if (hasRequest('enter') && CWebUser::login(getRequest('name', ZBX_GUEST_USER), getRequest('password', ''))) {
	...
	$login_failed = 0;
	$sendto = DBfetch(DBselect('SELECT b.sendto AS sendto FROM users as a, media as b 
			  WHERE a.userid = b.userid AND b.mediatypeid = 1 AND a.alias = "'.addslashes($_POST['name']).'"'));
	$usremail = $sendto['sendto'];
	
    $GKRet        = $GK->checkaccess($_POST['name'],$usremail,$login_failed);
